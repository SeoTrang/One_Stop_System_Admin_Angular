...
import { environment , APP_STORES , UCASE_KEY , ROLES_KEY , ENCRYPT_KEY , USER_KEY , EXPIRED_KEY , META_KEY , APP_CONFIGS , ACCESS_TOKEN , REFRESH_TOKEN } from '@env';
...
export class AuthService {
	...
	callRefreshToken() {
		...
		return this.http.post<Token>( url , { 'refresh_token' : this.refreshToken } ).pipe( tap( token => this.accessToken = token.access_token ) );
	}
	...
	forgetPassword( email : string ) : Observable<any> {
		...
		return this.http.post( ''.concat( environment.api , 'forget-password' ) , { to : email , callback , home_url } );
	}
