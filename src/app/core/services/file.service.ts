...
import { OvicFileUpload , FileDto , OvicFileStore , OvicDriveFile , OvicFileSever , Download , Upload , OvicDriveFolder } from '@core/models/file';
import { Observable , of } from 'rxjs';
import { distinctUntilChanged , filter , map , retry , scan } from 'rxjs/operators';
...
export class FileService {
	...
	private readonly media       = environment.media;
	private readonly dir         = environment.fileDir;
	private readonly download    = environment.download;
	private readonly googleDrive = environment.googleDrive;
	 ...
	 * ********************************************************/
	private static packetFiles( files : File[] ) : FormData {
	...
	ovicFileInfoToOvicFileUpload( files : OvicFileSever[] ) : OvicFileUpload[] {
	...
	getUploadDate( file : OvicFileSever | OvicDriveFile ) : string {
	...
	/**********************************************************
	 * find and search files functions
	 * ********************************************************/
	getFileInfo( ids : string ) : Observable<OvicFileSever[]> {
	...
	getFileList( params : any ) : Observable<OvicFileSever[]> {
	...
	getFileSize( file : OvicFileSever | OvicFileStore | OvicDriveFile ) : string {
	...
	getFileContentAsBlob( name_or_id : string ) : Observable<Blob> {
		return this.http.get( ''.concat( this.download , name_or_id ) , { responseType : 'blob' } );
	}
	...
	// 	return this.http.post<Dto>( this.googleDrive , FileService.packetFiles( [ file ] ) ).pipe( map( res => res.data || [] ) );
	// }
	...
	googleDriveUploadFileToParents( file : File , parents : string ) : Observable<OvicDriveFile[]> {
		...
		return this.http.post<Dto>( this.googleDrive , FileService.packetFiles( [ file ] ) , { headers } ).pipe( map( res => res.data || '' ) );
	}
	...
	uploadFileOfDonviNew( files , donvi_id : number , user_id : number ) : Observable<any> {
		const params = new HttpParams().set( 'donvi_id' , donvi_id.toString() ).set( 'user_id' , user_id.toString() );
		return this.http.post<Dto>( this.media , FileService.packetFiles( files ) , { params : params } );
	}

	/********************************************************************************
	 * Download file with progress
	 * ******************************************************************************/
	downloadUnionFileProgress( file : OvicFileSever | OvicDriveFile ) : Observable<Download> {
	 ...
	 * ******************************************************************************/
	downloadUnionFile( file : OvicFileSever | OvicDriveFile ) : Observable<Download> {
	 ...
	 * ******************************************************************************/
	getUnionFileProgress( file : OvicFileSever | OvicDriveFile ) : Observable<Download> {
	 ...
	 * ******************************************************************************/
	getUnionFile( file : OvicFileSever | OvicDriveFile ) : Observable<string> {
