...
export class LoginComponent implements OnInit {
	...
	loginEvent$ = new Subject<UserSignIn | GoogleSignIn>();
	...
	checkLogin( info : UserSignIn | GoogleSignIn ) {
		this.isLoading                    = true;
	...
	async handleGoogleSignIn( response : any ) {
		...
		const signInfo    = { id : token.sub , email : token.email , idToken : response.credential , provider : 'GOOGLE' };
		await this.ngZone.run( () => this.checkLogin( signInfo ) );
	}
}
