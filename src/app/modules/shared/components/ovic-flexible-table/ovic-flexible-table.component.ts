...
export class OvicFlexibleTableComponent implements OnInit , OnDestroy , OnChanges {
	...
	ngOnInit() : void {
		...
		const observeCloseTemplates = this.notificationService.onCloseOvicFlexibleTemplates.asObservable().pipe( debounceTime( 100 ) ).subscribe( message => this.__closeBox( message ) );
		this.subscriptions.add( observeCloseTemplates );

		const observeOpenTemplates = this.notificationService.onOpenOvicFlexibleTemplate.asObservable().pipe( filter( templateName => !!( templateName ) ) ).subscribe( templateName => this.openBox( templateName ) );
