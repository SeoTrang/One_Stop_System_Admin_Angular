import { Component , OnInit , ViewChild , Input , Output , EventEmitter , TemplateRef , ViewContainerRef , OnDestroy } from '@angular/core';
import { Table } from 'primeng/table';
import { OvicTableStructure , InsideAction } from '@shared/models/ovic-models';
...
export class OvicTableComponent implements OnInit , OnDestroy {
	...
	constructor(
		...
		const observerEventClick            = this._EVENT_CLICK.asObservable().pipe( debounceTime( 100 ) ).subscribe( {
			next : button => this.onButtonClick.emit( button )
		} );
