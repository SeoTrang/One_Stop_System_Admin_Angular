...
import { OvicFileStore , OvicFileSever } from '@core/models/file';
...
export class OvicFileManagerComponent implements OnInit , OnChanges {
	...
	convertToOvicFileSever( files : OvicFileStore[] ) : OvicFileSever[] {
		const files_result : OvicFileSever[] = [];
	...
	returnData( files : OvicFileSever[] ) {
		if ( this.formField ) {
