...
import { OvicDriveFile , OvicFileSever , OvicTinyDriveFile } from '@core/models/file';
...
export class OvicFileListComponent implements OnInit , OnChanges {
	...
	@Input() fileList$ : Subject<OvicDriveFile[] | OvicFileSever[]>;

	@Input() fileList : OvicDriveFile[] | OvicFileSever[] | OvicTinyDriveFile[];
	...
	removeFile( file : OvicDriveFile | OvicFileSever ) {
		this._fileList = this._fileList.filter( f => f.id !== file.id );
		this.outputFiles.emit( this._fileList );
	}

	async fileDetail( file : OvicDriveFile | OvicFileSever | OvicTinyDriveFile ) {
		if ( this.preview ) {
