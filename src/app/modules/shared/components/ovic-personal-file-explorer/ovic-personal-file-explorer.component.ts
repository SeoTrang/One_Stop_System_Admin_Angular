...
import { OvicDriveFile , OvicFileSever , OvicFileStore , OvicFileUpload , OvicTree } from '@core/models/file';
...
export class OvicPersonalFileExplorerComponent implements OnInit , OnDestroy {
	...
	// library : OvicFileSever[] | OvicDriveFile[] = [];
	library : any[];
	...
	_fileRunning : OvicFileSever | OvicDriveFile;
	 ...
	 * *****************************************************/
	private getData( serverName : string , search ) : Observable<OvicFileSever[] | OvicDriveFile[]> {
	...
	loadData() {
		...
		let getData$ : Observable<OvicFileSever[] | OvicDriveFile[]> = this.getData( this.server , this.searchInfo );
	 ...
	 * removeFileFromList
	 * *****************************************************/
	removeFileFromList( file : OvicFileSever | OvicDriveFile ) {
	 ...
	 * addFileToList
	 * *****************************************************/
	private addFileToList( file : OvicFileSever | OvicDriveFile ) {
	 ...
	 * selectFile
	 * *****************************************************/
	selectFile( file : OvicFileSever | OvicDriveFile ) {
	...
	validateOpenFolder( file : OvicDriveFile | OvicFileSever ) {
		if ( file.hasOwnProperty( 'mimeType' ) && file[ 'mimeType' ] === 'application/vnd.google-apps.folder' ) {
	 ...
	 * *****************************************************/
	async deleteFile( file : OvicFileSever | OvicDriveFile = null ) {
		if ( !this.canDelete ) {
			return;
	 ...
	 * ************************************************/
	async handleClickOnContextMenu( event : Event , file : OvicFileSever | OvicDriveFile , func : RightContextMenuFunction = null ) {
		event.preventDefault();
		event.stopPropagation();
		this.closeContextMenu();
		if ( !func ) {
	 ...
	 * *******************************************/
	async changeFilePublicationStatus( file : OvicFileSever | OvicDriveFile , func : RightContextMenuFunction ) {
	...
	openContextMenu( event : MouseEvent | TouchEvent , file ) {
		...
		const positionStrategy           = this.overlay.position().flexibleConnectedTo( { x : clientX , y : clientY } ).withPositions( [ { originX : 'end' , originY : 'bottom' , overlayX : 'end' , overlayY : 'top' } ] );
	 ...
	 * Download file act
	 * ******************************************************/
	async downloadFile( file : OvicFileSever | OvicDriveFile ) {
	 ...
	 * context menu functions
	 * ************************************************/
	async handleCopyAudioCode( event : Event , file : OvicFileSever | OvicDriveFile ) {
		event.preventDefault();
		event.stopPropagation();
	...
	isAudioFile( file : OvicFileSever | OvicDriveFile ) : boolean {
	...
	createNewStore() {
		...
		this.fileService.createPersonalFolder( { name , parents } ).pipe(
			mergeMap( folderInfo => {
				...
				return this.userService.createNewMeta( {
